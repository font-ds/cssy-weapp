...
export default function Index() {
  ...
  let wxInfo = Taro.getStorageSync('info')
  console.log(wxInfo)
    ...
    return (
      <View className='mime'>
        ...
        <View className='top-view'>
          <AtAvatar className='avator' circle size='large' image={wxInfo?.userInfo.avatarUrl}></AtAvatar>
          ...
          <View className='vip-view'>
            <Text className={info.power ? 'vip-type normal-type' : 'vip-type'} >{info?.power == 1 ? 'VIP' : info?.power == 2 ? 'SVIP' : '普通'}会员<Text className='hr'></Text><Text className='vip-time font-default-style'>{ getCurrentTime(info.power_expired*1000) }到期</Text></Text>
