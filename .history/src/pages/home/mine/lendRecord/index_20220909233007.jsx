          ...
          record.map(item=>
            <View className='record-view' key={item.time+item.type}>
              ...
              <View className='btn-view'>
                <View>
                  <Image src={state?state==1?yellow:green:blue}></Image>
                  <Text >{state?state==1?'借阅中':'已完成':'带取书'}</Text> 
                </View>
                <AtButton className='record-btn btn-concel'  type='primary' onClick={()=>longTimeModal(borrow_id)}>延长订阅</AtButton>
              </View>
            </View>
          )
        }
