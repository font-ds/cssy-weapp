...
export default function Index () {
  ...
  const info = useSelector(state => state.info.userInfo)
    ...
    return (
      <View className='power'>
        ...
        <View className='vip-view'>
          <View className='time'>
            <Text  className={info.power?'vip-type normal-type':'vip-type'}>{info?.power==1?'VIP':info?.power==2?'SVIP':'当前身份'}</Text>
            <Text className='vip-time font-default-style'>有效至:{ getCurrentTime(info.power_expired*1000)}</Text>
          </View>
          <View className={info.power?'vip-type normal-type':'vip-type'}>
            {info?.power==1?'VIP':info?.power==2?'SVIP':'普通会员'}
          </View>
