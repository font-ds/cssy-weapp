...
export default function AppointDetail() {
  ...
  if(type==0) data.date= getAppointCurrent(new Date().getTime()+data.date*24000*3600)
  else if (type == 2) { 
  ...
  const appoint=()=>{
    ...
    // 导读师预约
      ...
      let periods = new Array(24).fill(false)
      data.periods.map(item => { 
        if (item === 1) return periods[item]=true
      })
      request({
        url: '/front/tutor/reserve',
        method:'post',
        data: {
        ...
        <View className='message'>
          ...
          <View>预约时间：{
            type != 3 ? type == 2 ? data.date + ' ' + data.periods.map(item => { 
              return `${10 + Math.floor(item / 2)}:${0 + (item % 2) * 3}0-${10 + Math.floor((item + 1) / 2)}:${0 + ((item + 1) % 2) * 3}0`
            }) :
