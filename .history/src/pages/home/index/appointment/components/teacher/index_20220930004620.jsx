            ...
            <View className='teacher-view'>
              <View className='teacher-info'>
                ...
                <View className='info-right' onClick={() => { 
                  console.log(1111)
                  dispatch(set({url:item.avatar,times:item.day,content:item.introduction}))
                  let newData = encodeURIComponent(JSON.stringify({ tutor_id: item.tutor_id, name: item.name }))
                  Taro.navigateTo({url: '/pages/home/index/appointment/teacherAppoint/index?data='+newData })
                }}
                >
                <Text>+</Text> 预约讲师
              </View>
