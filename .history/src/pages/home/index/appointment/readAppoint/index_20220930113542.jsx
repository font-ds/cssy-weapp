...
export default function Reading () {
  ...
  const next=(period)=>{
    let data={
      ...
      detailTime: topChoose[swiperCurrent].detailTime,
      dayIndex:swiperCurrent,
      period
    }
    if(title=='亲子预约') data.type=0
    else data.type=1

    Taro.navigateTo({url:`/pages/home/index/components/appointDetail/index?type=0&data=${JSON.stringify(data)}`})
  }
    ...
    return (
      <View className='read-appoint font-default-style'>
        ...
        >
          {
            info.content.Payload.map((item,index)=>(
              <SwiperItem style={{height:'100%'}} key={index}>
                ...
                <ScrollView  scrollY enhanced showScrollbar={false} className='scroll-view'>
                  {
                    item.map((timeItem,timeIndex)=>
                      <View key={timeItem.number+timeIndex} className='choose-hour' onClick={()=>next(timeIndex)}>
                        <View className='hour-left-view'>
                          <View className='circle'></View>
                          <View className='hour-left'>{10+timeItem.Period*2}:00--{12+timeItem.Period*2}:00</View>
                        </View>
                        <View className='num'>
                          <View>已预约</View>
                          <View>{ timeItem.Number}人</View>
                        </View>
                      </View>
                    )
                  }
