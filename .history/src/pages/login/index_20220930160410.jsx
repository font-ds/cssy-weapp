        ...
        request({
          ...
          success:function(res){
            ...
            // if(res.statusCode==200){
              if(res.status==1) setPsType(1)
              else if (res.status == 2) setPsType(2)
                else if(res.status==5) setPsType(4)
  ...
  return (
    <View className='login'>
        ...
        <Text className='getCode font-default-style'
          ...
          onClick={getCode}
        >{downSecond?downSecond+'s后重试':'获取验证码'}</Text>
        <Text className='login-ps'>
          {!psType?null:psType==1?'会员账号不存在，请重试':psType==2?'验证码错误，请重试':psType==3?'请输入正确的手机号':'登录失败，请重试'}
        </Text>
